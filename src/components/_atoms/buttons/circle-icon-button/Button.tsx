...
const Button: FC<IButtonProps> = ({ disabled = false, onClick, icon, type, title, quanttiy=0, size = 'lg', className = styles.container__button }): ReactElement => {
    ...
    <div className={styles.container}>
      <button type={type} ref={butonRef} disabled={disabled} className={`${styles.container__button} ${sizeClass}`} onClick={onClick}>
        <span className={styles.container__button__icon}>
          {quanttiy > 0 && <div className={styles.container__button__badge}>{quanttiy}</div>}
          {icon}
        </span>
        {title && (
          <span
